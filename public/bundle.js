...
(0, _fetchPupilRecords.fetchPupilRecords)().then(function (pupils) {
	...
	active = (0, _fetchPupilRecords.filterActive)(_stateObject.state.pupilRecordsArray);
	_stateObject.state.days.forEach(function (day) {
		var todayReg = dailyRegister(day);
		makeDay(todayReg, day);
	});
});
function dailyRegister(day) {
	var dayReg = active.filter(function (pup) {
		return pup.days.indexOf(day) > -1;
	});
	return dayReg;
}
function makeDay(todayReg, day) {
	var table = (0, _generateTimetable.getTemplate)();
	var todayNames = getFNames(todayReg);
	var temp = table.dayBox;
	var header = table.dayHead;
...
function getTemplate() {
	...
	var pupilAge = document.querySelector('#pupilAge');
	var tableParts = {};
	...
	tableParts.dayHead = tableParts.dayBox.querySelector('#dayHeading');
	return tableParts;
}
