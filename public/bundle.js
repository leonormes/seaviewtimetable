...
(0, _fetchPupilRecords.fetchPupilRecords)().then(function (pupils) {
	...
	active = (0, _fetchPupilRecords.filterStatus)(_stateObject.state.pupilRecordsArray, 'Active');
	waiting = (0, _fetchPupilRecords.filterStatus)(_stateObject.state.pupilRecordsArray, 'Waiting');
	_stateObject.state.days.forEach(function (day) {
		var todayReg = dailyRegister(day, active);
		var todayWait = dailyRegister(day, waiting);
...
function dayWaiting(todayWaiting, day) {
	var waitingNames = getFNames(todayWaiting);
	var temp = tableRowContent(waitingNames);
...
function makeDay(todayReg, day) {
	var todayNames = getFNames(todayReg);
	_stateObject.state.spaces[day] = 8 - todayNames.length;
	var header = _stateObject.state.tableParts.dayHead;
	header.textContent = day;
	var temp = tableRowContent(todayNames);
	var tab = document.importNode(temp, true);
	tab.id = day;
...
function tableRowContent(todayNames) {
	...
	var row = temp.querySelectorAll('.dayRow');
	for (var i = 0; i < 8; i++) {
		if (todayNames[i]) {
			row[i].textContent = todayNames[i].fname;
			row[i].id = todayNames[i].key;
			row[i].appendChild(addAgeDiv(todayNames[i].dob, _stateObject.state.tableParts.age));
		} else {
...
},{"./fetchPupilRecords.js":3,"./generateTimetable":4,"./stateObject":5,"moment":1}],3:[function(require,module,exports){
'use strict';
...
module.exports = { fetchPupilRecords: fetchPupilRecords, pupilRecordsToArray: pupilRecordsToArray, filterStatus: filterStatus };
...
},{"./stateObject":5}],5:[function(require,module,exports){
'use strict';
